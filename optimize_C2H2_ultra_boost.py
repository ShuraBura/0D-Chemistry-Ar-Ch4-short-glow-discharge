#!/usr/bin/env python3
"""
ULTRA C2H2 BOOST + C2 PROTECTION STRATEGY

Key changes from previous optimization:
1. C2H2 target: 3.5e12 → 7.0e12 (2× boost)
2. C2 weight: 15.0 → 20.0 (stronger emphasis)
3. CH weight: 5.0 → 3.0 (more tolerance for deviation)
4. C2_H_CH_C rate specifically targeted for suppression

Strategy:
- Push C2H2 even higher to maximize C2H2 + H → C2 production (438 THz/cm³)
- Suppress C2 + H → CH + C to prevent C2 loss and CH gain (119 THz/cm³)
- Allow optimizer to explore lower H densities to reduce CH2 + H → CH
"""
...
# Experimental targets - ULTRA C2H2 BOOST
TARGETS = {
    'H': 5.18e13,
    'CH': 1.0e9,
    'C2': 1.3e11,
    'C2H2': 7.0e12,  # ← 2× BOOST! Push C2H2 + H → C2 pathway even harder
}

# Create results directory
os.makedirs('optimization_results_C2H2_ultra', exist_ok=True)
...
def select_tunable_rates():
    """
    Select tunable rates - emphasize C2/CH chemistry control.
    """
    # Critical neutral reactions - EMPHASIZE C2 protection
    critical_neutral = [
        'CH2_H_CH_H2_cm3_7_1',          # CH2+H→CH (major CH source: 279 THz)
        'C2_H_CH_C_cm3_7_6',            # C2+H→CH+C ← SUPPRESS THIS! (119 THz problem)
        'C_H_CH_cm3_7_12',              # C+H→CH
        'C2H2_H_C2_H2_H_cm3_7_50',     # C2H2+H→C2 ← BOOST THIS! (438 THz)
        'CH_CH4_C2H4_H_cm3_7_20',      # CH+CH4→C2H4+H (CH sink)
        'CH_CH4_CH2_CH3_cm3_7_39',     # CH+CH4→
        'CH_H_C_H2_cm3_7_3',            # CH+H→C+H2 (CH loss)
        'CH2_CH3_C2H2_H_H2_cm3_7_62',  # CH2+CH3→C2H2 (C2H2 production)
        'CH3_CH3_C2H2_H2_H2_cm3_7_49', # CH3+CH3→C2H2 (major C2H2 source)
    ]
...
def objective_function(x, param_names, params_base):
    """Objective function with ULTRA C2H2 boost strategy."""
    ...
    # ULTRA C2H2 boost weights - aggressive rebalancing
    weights = {
        'H': 1.0,
        'CH': 3.0,   # ← REDUCED: More tolerance, allow lower H conditions
        'C2': 20.0,  # ← INCREASED: Primary objective - must match!
        'C2H2': 8.0  # ← INCREASED: Strong push to feed C2H2 + H → C2 pathway
    }
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_C2H2_ultra/best_f{total_error:.1f}_Te{Te:.2f}.json'
...
def main():
    ...
    print("=" * 80)
    print(" ULTRA C2H2 BOOST + C2 PROTECTION STRATEGY")
    print("=" * 80)
    print("\nStrategy:")
    print("  1. C2H2 target: 7.0e12 cm⁻³ (2× boost from previous)")
    print("  2. C2 weight: 20.0 (must match!)")
    print("  3. CH weight: 3.0 (more tolerance)")
    print("  4. Target C2+H→CH+C rate for suppression")
    print("\nGoals:")
    print("  → Maximize C2H2 + H → C2 production (438+ THz/cm³)")
    print("  → Minimize C2 + H → CH + C loss (reduce from 119 THz/cm³)")
    print("  → Reduce CH overshoot (currently 6.7× target)")
    ...
    print(f"\n Targets:")
    print(f"  H:    {TARGETS['H']:.2e} cm^-3  (weight: 1.0)")
    print(f"  CH:   {TARGETS['CH']:.2e} cm^-3  (weight: 3.0)")
    print(f"  C2:   {TARGETS['C2']:.2e} cm^-3  (weight: 20.0) ← PRIMARY")
    print(f"  C2H2: {TARGETS['C2H2']:.2e} cm^-3  (weight: 8.0) ← ULTRA BOOST")
    ...
    print("  • C2H2 + H → C2 pathway: 438 → 600+ THz/cm³")
    print("  • C2 + H → CH + C loss: 119 → <50 THz/cm³")
    print("  • CH overshoot: 6.7× → <5× target")
    print("  • C2 match: 0.37× → >0.5× target\n")
    ...
    with open('optimization_results_C2H2_ultra/FINAL_RESULT.json', 'w') as f:
        json.dump({
    ...
    print(f"\n✓ Final parameters saved to optimization_results_C2H2_ultra/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_C2H2_ultra/best_*.json")
