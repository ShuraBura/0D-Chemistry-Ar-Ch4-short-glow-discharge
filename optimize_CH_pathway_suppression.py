#!/usr/bin/env python3
"""
TARGETED CH-SUPPRESSION VIA PATHWAY CONTROL

Starting from good H/C2 balance (H=0.76×, C2=1.39×), need to reduce CH from 33× to <1.4×

Real Targets: H, CH, C2 within 0.6-1.4× range + good charge balance
C2H2: NOT a target, just a tool to boost C2

Strategy - Direct Chemistry Control:
1. FORCE high CH wall sticking (stick_CH) - increase loss
2. SUPPRESS electron-impact CH production (e+CH4→CH)
3. CONTINUE suppressing C2+H→CH pathway
4. Maintain C2H2→C2 production pathway (keep C2 high)
5. Tunable H sheath source

Weights focused on 3 real targets:
- H: 15, CH: 80, C2: 60 (C2H2: minimal ~1, just a tool)
...
# C2H2 is NOT a target, just a tool to boost C2
TARGETS = {
    'H': 5.18e13,     # ← Must be 0.6-1.4× (3.1e13 to 7.3e13)
    'CH': 1.0e9,      # ← Must be 0.6-1.4× (6e8 to 1.4e9) - currently 33×!
    'C2': 1.3e11,     # ← Must be 0.6-1.4× (7.8e10 to 1.8e11) - currently good at 1.39×
    'C2H2': 3.0e12,   # ← NOT a real target, just a tool for C2
}

# Create results directory
os.makedirs('optimization_results_CH_suppression', exist_ok=True)
...
def objective_function(x, param_names, params_base):
    ...
    # FOCUSED WEIGHTS - Only 3 real targets: H, CH, C2
    # C2H2 is minimal since it's not a real target
    weights = {
        'H': 15.0,     # Moderate - need 0.6-1.4× range
        'CH': 80.0,    # MAXIMUM! Need to reduce from 33× to 0.6-1.4×
        'C2': 60.0,    # High - maintain 0.6-1.4× (currently good at 1.39×)
        'C2H2': 1.0    # Minimal - NOT a real target, just a tool
    }
    ...
    # EXPLICIT pathway penalty for C2+H→CH flux (normalize by 1e14)
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_CH_suppression/best_f{total_error:.1f}_Hdrift{H_drift_gain:.2e}.json'
...
def main():
    print("=" * 80)
    print(" TARGETED CH-SUPPRESSION VIA PATHWAY CONTROL")
    print("=" * 80)
    print("\nStarting point: H=0.76×✓, C2=1.39×✓, but CH=33.61×✗")
    print("\nReal targets: H, CH, C2 within 0.6-1.4× + charge balance <100%")
    print("C2H2: NOT a target, just a tool to boost C2\n")
    print("Strategy - Direct Chemistry Control:")
    print("  1. FORCE high CH wall sticking (stick_CH) - increase CH loss")
    print("  2. SUPPRESS electron-impact CH production (e+CH4→CH)")
    print("  3. CONTINUE suppressing C2+H→CH pathway (already penalized)")
    print("  4. Maintain C2H2→C2 production (keep C2 high)")
    print("  5. Tunable H sheath source")
    print("\nWeights: H:15, CH:80 (MAX!), C2:60, C2H2:1 (minimal)")
    print("Charge penalty: 20× (doubled)")
    print("\nGoal: H, CH, C2 all within 0.6-1.4× range, charge balance good")
    ...
    print(f"\n Real Targets (must be 0.6-1.4×):")
    print(f"  H:    {TARGETS['H']:.2e} cm^-3  (weight: 15.0) → need 0.6-1.4×")
    print(f"  CH:   {TARGETS['CH']:.2e} cm^-3  (weight: 80.0) ← MAXIMUM! Reduce from 33×")
    print(f"  C2:   {TARGETS['C2']:.2e} cm^-3  (weight: 60.0) → maintain 0.6-1.4×")
    print(f"  C2H2: {TARGETS['C2H2']:.2e} cm^-3  (weight: 1.0) ← NOT a real target, just a tool")
    print(f"\n  + Charge balance (weight: 20.0) ← DOUBLED")
    print(f"  + C2+H→CH flux penalty (weight: 2.0) ← Suppress CH production")

    print("\n" + "=" * 80)
    print(" Running Optimization (20 iterations, pop=8)")
    print("=" * 80)
    print("\nTarget: H, CH, C2 all within 0.6-1.4× range")
    print("Strategy: Maximum CH pressure + pathway control + good charge balance\n")
