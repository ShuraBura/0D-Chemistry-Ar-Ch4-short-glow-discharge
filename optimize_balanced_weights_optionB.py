#!/usr/bin/env python3
"""
OPTION B: BALANCED WEIGHTS WITH C2-PRIORITY

Rebalanced weight strategy to prevent C2/C2H2 collapse:
1. CH weight: 40 (aggressive but reduced from 50)
2. C2 weight: 60 (HIGHEST - increased from 30, 2× boost)
3. C2H2 weight: 40 (equal to CH - massively increased from 5, 8× boost!)
4. Tunable H sheath source - decouples H from chemistry
5. EXPLICIT penalty for C2+H→CH+C pathway flux

Philosophy: Create tension - optimizer must satisfy ALL targets simultaneously
Cannot sacrifice C2/C2H2 to reduce CH anymore!

Goal: CH near 1×, C2 near 1×, C2H2 near 1×, H near 1×, f < 100
"""
...
os.makedirs('optimization_results_optionB', exist_ok=True)
...
def objective_function(x, param_names, params_base):
    ...
    # OPTION B: BALANCED WEIGHTS - C2 PRIORITY
    weights = {
        'H': 5.0,      # Moderate - from tunable sheath source
        'CH': 40.0,    # Aggressive (reduced from 50) - still strong pressure
        'C2': 60.0,    # HIGHEST! (increased from 30, 2× boost) - prevent collapse
        'C2H2': 40.0   # Equal to CH! (increased from 5, 8× boost) - cannot be sacrificed
    }
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_optionB/best_f{total_error:.1f}_Hdrift{H_drift_gain:.2e}.json'
...
def main():
    print("=" * 80)
    print(" OPTION B: BALANCED WEIGHTS WITH C2-PRIORITY")
    print("=" * 80)
    print("\nRebalanced weight strategy to prevent C2/C2H2 collapse:")
    print("  1. CH weight: 40 (aggressive but reduced from 50)")
    print("  2. C2 weight: 60 (HIGHEST - increased from 30, 2× boost)")
    print("  3. C2H2 weight: 40 (equal to CH - increased from 5, 8× boost!)")
    print("  4. Tunable H sheath source + explicit C2+H→CH flux penalty")
    print("\nPhilosophy: Create tension - optimizer must satisfy ALL targets!")
    print("Cannot sacrifice C2/C2H2 to reduce CH anymore!")
    print("\nGoal: CH ~1×, C2 ~1×, C2H2 ~1×, H ~1×, f < 100")
    ...
    print(f"  CH:   {TARGETS['CH']:.2e} cm^-3  (weight: 40.0) ← Aggressive")
    print(f"  C2:   {TARGETS['C2']:.2e} cm^-3  (weight: 60.0) ← HIGHEST! Prevent collapse")
    print(f"  C2H2: {TARGETS['C2H2']:.2e} cm^-3  (weight: 40.0) ← Equal to CH! Cannot sacrifice")
    ...
    print("=" * 80)
    print("\nTarget: CH ~1×, C2 ~1×, C2H2 ~1×, H ~1×, f < 100")
    print("Strategy: Strong equal pressure on CH/C2H2, maximum pressure on C2!\n")
