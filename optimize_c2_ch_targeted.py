#!/usr/bin/env python3
"""
TARGETED optimization with EXPANDED tunable rates.

KEY CHANGE: Now includes critical neutral reactions that were MISSING:
...
This allows optimizer to minimize C2+H→CH while building up C2.
Result: C2↑, CH↓ simultaneously!
"""
...
os.makedirs('optimization_results_targeted', exist_ok=True)
...
def select_tunable_rates():
    ...
    # Combine: critical neutral reactions + previous selections
...
def objective_function(x, param_names, params_base):
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_targeted/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
...
def main():
    ...
    print("=" * 80)
    print(" FULL 32-PARAMETER OPTIMIZATION WITH COMPREHENSIVE LOGGING")
    print("=" * 80)
    print("\nWarm start from previous best: f(x) = 1097 (step 4)")
    print("Strategy: Ne↓, E↑, reduce C2H2→C2→CH pathway rates")
    ...
    print("=" * 80)
    print("\nDetailed logs will be saved to optimization_results_targeted/")
    ...
    with open('optimization_results_targeted/FINAL_RESULT.json', 'w') as f:
        json.dump({
    ...
    print(f"\n✓ Final parameters saved to optimization_results_targeted/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_targeted/best_iteration_*.json")
