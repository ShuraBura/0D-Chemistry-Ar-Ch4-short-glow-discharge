...
CHARGE_BALANCE_WEIGHT = 50.0  # REDUCED from 200 to allow CH optimization!

# Create results directory
os.makedirs('optimization_results_ch_focused_v4', exist_ok=True)
...
def objective_function(x, param_names, params_base):
    ...
    weights = {
        'H': 1.0,
        'CH': 40.0,  # DOUBLED from 20 to prioritize CH optimization!
        'C2': 3.0
    }
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_ch_focused_v4/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
...
def main():
    ...
    print("=" * 80)
    print(" V4 OPTIMIZATION - CH-FOCUSED STRATEGY")
    print("=" * 80)
    print("\nV4 STRATEGY (per user request - PRIORITIZE CH!):")
    print("  ✓ CH weight: 40x (DOUBLED from 20x)")
    print("  ✓ Charge penalty: 50x (REDUCED from 200x)")
    print("  ✓ 101 tunable rates (proven effective in V3)")
    print("  ✓ ALL wall sticking + volume loss rates")
    print("  ✓ Most electron-impact (no ionization)")
    print("  ✓ Ionization rates FIXED (per user guidance)")
    print("\nV3 best: CH = 5.82x (f=618), but ion/e = 1.10x")
    print("V4 goal: Prioritize CH reduction → get closer to 1.0x!")
    ...
    print("=" * 80)
    print("\nDetailed logs will be saved to optimization_results_ch_focused_v4/")
    ...
    with open('optimization_results_ch_focused_v4/FINAL_RESULT.json', 'w') as f:
        json.dump({
    ...
    print(f"\n✓ Final parameters saved to optimization_results_ch_focused_v4/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_ch_focused_v4/best_iteration_*.json")
