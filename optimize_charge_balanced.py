...
CHARGE_BALANCE_WEIGHT = 5.0  # Penalty weight for charge imbalance
...
def select_tunable_rates():
    """
    EXPANDED tunable rates including critical neutral reactions!

    Key additions (MISSING before):
    - CH2+H→CH (48.7% of CH production)
    - C2+H→CH (24% of CH production) ← KEY RATE!
    - C+H→CH (12.9% of CH production)
    - C2H2+H→C2 (96.5% of C2 production)
    - CH+CH4 reactions (CH loss)
    """
    # Critical neutral reactions that were MISSING from previous optimization
    critical_neutral = [
    ...
    # ION reactions affecting charge balance (NEW!)
    critical_ion = [
        'e_CH4_CH3Plus_H_cm3_2_1',      # e+CH4→CH3+ (ionization)
        'e_CH4_CH4Plus_cm3_2_2',        # e+CH4→CH4+ (ionization)
        'e_Ar_ArPlus_cm3_2_3',          # e+Ar→Ar+ (ionization)
        'e_ArStar_ArPlus_cm3_2_4',      # e+Ar*→Ar+ (Penning-like)
        'ArPlus_CH4_Ar_CH4Plus_cm3_5_5',  # Ar+ + CH4 charge transfer
    ...
    # Combine: critical neutral + critical ion + previous selections
    selected = set(critical_neutral) | set(critical_ion) | target_rates | large_range_rates | flagged_rates
    ...
    # Expand to top 40 rates (from 30)
...
def objective_function(x, param_names, params_base):
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_charge_balanced/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
...
def create_warm_start_population(param_names, bounds, db, popsize=6):
    """
    Create initial population biased toward previous best result.

    Previous best (step 4): f(x) = 1097
    - Ne ~ 1.5-2e9 (lower end)
    - E ~ 200-280 (higher end)
    - Key rates toward minimums: C2H2+H→C2, C2+H→CH
    """
    print("\n  Creating warm-start population from previous best (f=1097)...")
    ...
    for i in range(popsize):
        for j, (name, (low, high)) in enumerate(zip(param_names, bounds)):
            if name == 'E_field':
                # Bias toward higher E (200-280)
                population[i, j] = np.random.uniform(200, min(280, high))
            elif name == 'ne':
    ...
    print("  ✓ Population biased toward: Ne~1.5e9, E~250, key rates reduced")
    return population
...
def main():
    ...
    print("=" * 80)
    print(" CHARGE-BALANCED OPTIMIZATION WITH EXPANDED RATES")
    print("=" * 80)
    print("\nNEW FEATURES:")
    print("  ✓ Charge balance constraint (target ion/e = 4.5x)")
    print("  ✓ Lower E field minimum (5 V/cm to reduce ion loss)")
    print("  ✓ Ion reaction rates now tunable")
    print("  ✓ Critical neutral reactions included")
    print("\nStrategy: Lower E → retain ions → achieve charge balance")
    ...
    print("=" * 80)
    print("\nDetailed logs will be saved to optimization_results_charge_balanced/")
    ...
    print(f"\n✓ Final parameters saved to optimization_results_charge_balanced/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_charge_balanced/best_iteration_*.json")
