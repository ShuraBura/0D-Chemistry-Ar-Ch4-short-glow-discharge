#!/usr/bin/env python3
"""
COMPREHENSIVE OPTIMIZATION WITH CHARGE BALANCE - FIXED C2 WEIGHTS

CRITICAL FIX for C2 collapse issue:
1. C2 weight increased from 3.0 to 20.0 (equal to H and CH)
2. This prevents optimizer from sacrificing C2 to improve H/CH
3. Force-includes C2-producing reactions (e+C2H2→C2, CH+CH→C2, etc.)
4. Charge balance penalty in objective function (Ni/Ne in [2, 7] range)
5. Ne constrained to [2.0e9, 3.0e9] cm⁻³

Previous issue: C2 had only 3.0 weight vs 20.0 for H/CH, so optimizer
maximized C2 wall loss to push carbon into other species.
"""
...
# Create results directory for this fixed version
os.makedirs('optimization_results_charge_balanced_fixed_weights', exist_ok=True)
...
def objective_function(x, param_names, params_base):
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_charge_balanced_fixed_weights/best_f{total_error:.1f}.json'
