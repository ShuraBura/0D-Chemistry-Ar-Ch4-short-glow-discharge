#!/usr/bin/env python3
"""
COMPREHENSIVE OPTIMIZATION - INTEGRATES TO TRUE STEADY STATE

CRITICAL FIXES:
1. Integration time increased from t=100s to t=500s (reaches true steady state)
2. C2 weight = 20.0 (equal to H and CH)
3. Force-includes C2-producing reactions
4. Charge balance penalty (Ni/Ne in [2, 7])

Previous issue: t=100s integration stopped at transient state where:
  - H appeared to be 1e+13 (4% of target)
  - But TRUE steady state at t=1000s showed H = 3.66e+14 (145% of target!)
Optimizer was optimizing for the wrong state!

This version ensures optimizer sees true steady state values.
"""
...
# Create results directory for this steady-state version
os.makedirs('optimization_results_charge_balanced_steady_state', exist_ok=True)
...
def objective_function(x, param_names, params_base):
    ...
    # Species target errors - FIXED: Equal weights for all species!
    # Previous weights (H:20, CH:20, C2:3) caused optimizer to sacrifice C2
    # to improve H and CH. Now all species are equally important.
    weights = {
        'H': 20.0,
        'CH': 20.0,
        'C2': 20.0   # FIXED: Was 3.0, now equal to H and CH
    }
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_charge_balanced_steady_state/best_f{total_error:.1f}.json'
