...
os.makedirs('optimization_results_charge_balanced_v2', exist_ok=True)
...
def select_tunable_rates():
    ...
    # ION LOSS reactions affecting charge balance (CORRECTED!)
    # NOTE: DO NOT tune ionization rates per user guidance!
    # Only tune ion LOSS mechanisms (wall sticking, recombination)
    critical_ion = [
        'stick_ArPlus_9_4',              # Ar+ wall loss (REDUCE to retain ions)
        'stick_HMinus_9_18',             # H- wall loss
        'stick_CH3Minus_9_15',           # CH3- wall loss
        # Removed ionization rates - these should stay at literature values!
        # Removed: e+CH4→CH3+, e+Ar→Ar+, etc.
    ]
...
def objective_function(x, param_names, params_base):
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_charge_balanced_v2/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
...
def main():
    ...
    print("=" * 80)
    print(" CHARGE-BALANCED OPTIMIZATION V2 - CORRECTED STRATEGY")
    print("=" * 80)
    print("\nV2 CORRECTIONS (from v1 failure analysis):")
    print("  ✓ Charge penalty weight: 200x (was 5.0 - too weak!)")
    print("  ✓ Ionization rates FIXED (NOT tunable - per user guidance)")
    print("  ✓ Only ion LOSS rates tunable (wall sticking)")
    print("  ✓ E field biased LOW (10-50 V/cm to retain ions)")
    print("  ✓ Critical neutral reactions included")
    print("\nV1 failure: Optimizer minimized ionization → worse charge balance")
    print("V2 strategy: Fix ionization, minimize ion LOSS → better charge balance")
    ...
    print("=" * 80)
    print("\nDetailed logs will be saved to optimization_results_charge_balanced_v2/")
    ...
    with open('optimization_results_charge_balanced_v2/FINAL_RESULT.json', 'w') as f:
        json.dump({
    ...
    print(f"\n✓ Final parameters saved to optimization_results_charge_balanced_v2/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_charge_balanced_v2/best_iteration_*.json")
