#!/usr/bin/env python3
"""
Comprehensive optimizer targeting C2H2 ~ 1e12 (more modest than 5e12)
with EXTENSIVE reaction rate tuning to enable stable charge balance.

User's insight: Experiment achieves high C2H2 with stable plasma, so model must too.
Strategy: Tune many more reactions, especially those affecting electron/ion balance.
"""
...
os.makedirs('optimization_results_comprehensive_1e12', exist_ok=True)
...
# More modest C2H2 target
C2H2_target = 1e12  # User's suggestion: try ~1e12 instead of 5e12
...
def objective(x):
    ...
    # COMPREHENSIVE reaction list - tune MANY more reactions!
    critical_reactions = [
        # === C2 PRODUCTION ===
        ...
        # === C2 DESTRUCTION ===
        'C2_H_CH_C_cm3_7_6',             # H + C2 → CH + C (REDUCE!)

        # === C2 LOSSES ===
        'stick_C2_9_9',                  # C2 wall sticking
        'loss_C2_11_3',                  # C2 volumetric loss

        # === C2H2 LOSSES (critical for accumulation) ===
        'stick_C2H2_9_11',               # C2H2 wall sticking
        'loss_C2H2_11_19',               # C2H2 volumetric loss

        # === C2H2 PRODUCTION (boost!) ===
        'CH3_CH3_C2H2_H2_H2_cm3_7_49',   # 2CH3 → 2H2 + C2H2 (67%!)
        'C_CH3_C2_H2_H_cm3_7_8',         # C + CH3 → H + C2H2
        'CH3_CH_C2H2_H2_cm3_7_16',       # CH + CH3 → H2 + C2H2

        # === ELECTRON IMPACT IONIZATION (affects Ne) ===
        'e_Ar_ArPlus_2e_cm3_2_1',        # e + Ar → Ar+ + 2e
        'e_CH4_CH4Plus_2e_cm3_2_2',      # e + CH4 → CH4+ + 2e
        'e_CH4_CH3Plus_H_2e_cm3_2_3',    # e + CH4 → CH3+ + H + 2e

        # === ELECTRON-ION RECOMBINATION (balance!) ===
        'e_CH4Plus_CH3_H_cm3_6_4',       # CH4+ + e → CH3 + H
        'e_CH4Plus_CH2_H2_cm3_6_9',      # CH4+ + e → CH2 + H2
        'e_CH3Plus_CH2_H_cm3_6_6',       # CH3+ + e → CH2 + H
        'e_ArPlus_Ar_cm3_6_1',           # Ar+ + e → Ar

        # === PENNING IONIZATION (Ar* affects charge balance) ===
        'ArStar_CH4_CH3_H_cm3_3_1',      # Ar* + CH4 → CH3 + H (competes with ionization)
        'stick_ArStar_9_5',              # Ar* wall loss

        # === ELECTRON ATTACHMENT (creates negative ions) ===
        'e_CH4_CH3_HMinus_cm3_8_1',      # e + CH4 → CH3 + H-

        # === CH CONTROL (prevent accumulation) ===
        'loss_CH_11_9',                  # CH volumetric loss
        'stick_CH_9_3',                  # CH wall sticking

        # === CH3 CONTROL (affects C2H2 production) ===
        'stick_CH3_9_2',                 # CH3 wall sticking
        'loss_CH3_11_2',                 # CH3 volumetric loss
    ]
    ...
    try:
        ...
        # Charge balance constraint (CRITICAL!)
        if Ni_over_Ne < 2.0 or Ni_over_Ne > 7.0:
            charge_penalty = 200.0 * min((2.0 - Ni_over_Ne)**2, (Ni_over_Ne - 7.0)**2)
            f += charge_penalty

        # C2H2 target ~ 1e12 (more modest, bi-directional penalty)
        C2H2_err = ((C2H2_final - C2H2_target) / C2H2_target)**2
        C2H2_penalty = 50.0 * C2H2_err
        f += C2H2_penalty
        ...
        if f < best_f:
            ...
            filename = f'optimization_results_comprehensive_1e12/best_f{f:.1f}.json'
            ...
            print(f"  C2H2: {C2H2_final:.2e} ({C2H2_final/C2H2_target*100:5.1f}% of 1e+12 target)")
...
if __name__ == '__main__':
    print("=" * 80)
    print("COMPREHENSIVE OPTIMIZER: C2H2 ~ 1e12 + EXTENSIVE TUNING")
    print("=" * 80)
    print()
    print("Strategy:")
    print("  1. Target C2H2 ~ 1e12 (more modest than 5e12)")
    print("  2. Tune 30+ reactions including:")
    print("     - C2/C2H2 chemistry")
    print("     - Electron impact ionization")
    print("     - Electron-ion recombination")
    print("     - Penning ionization")
    print("     - Electron attachment")
    print("  3. Find conditions where BOTH C2H2 and charge balance work")
    print()
    ...
    rate_bounds = {
        ...
        # Electron impact ionization (wider bounds for flexibility!)
        'e_Ar_ArPlus_2e_cm3_2_1': (1e-11, 1e-09),
        'e_CH4_CH4Plus_2e_cm3_2_2': (1e-11, 1e-09),
        'e_CH4_CH3Plus_H_2e_cm3_2_3': (1e-12, 1e-10),

        # Electron-ion recombination
        'e_CH4Plus_CH3_H_cm3_6_4': (1e-08, 1e-06),
        'e_CH4Plus_CH2_H2_cm3_6_9': (1e-08, 1e-06),
        'e_CH3Plus_CH2_H_cm3_6_6': (1e-08, 1e-06),
        'e_ArPlus_Ar_cm3_6_1': (1e-08, 1e-06),

        # Penning ionization
        'ArStar_CH4_CH3_H_cm3_3_1': (1e-11, 1e-09),
        'stick_ArStar_9_5': (100, 2000),

        # Electron attachment
        'e_CH4_CH3_HMinus_cm3_8_1': (1e-14, 1e-12),
