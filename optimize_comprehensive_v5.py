...
CHARGE_BALANCE_WEIGHT = 75.0  # Moderate penalty (between V4's 50 and V3's 200)

# Create results directory
os.makedirs('optimization_results_comprehensive_v5', exist_ok=True)
...
def select_tunable_rates():
    """
    V5: COMPREHENSIVE - MAXIMUM tunable rates based on chemistry analysis!

    Strategy: Tune ALL reactions with range > 1.5x (~192 rates) PLUS
    critical secondary pathways identified from chemistry analysis.

    Targets for optimization:
    - H:  TOO LOW (0.40x) → INCREASE by reducing H loss
    - C2: TOO LOW (0.32x) → INCREASE by reducing C2 loss
    - CH: TOO HIGH (5.30x) → DECREASE (already at limits)

    Critical findings from chemistry:
    - H loss dominated by: H+CH4→CH3+H2 (75.4%) and H wall (23.4%)
    - C2 loss dominated by: C2 wall (63.4%), C2+H→CH (18.5%), C2 volume loss (18.1%)
    - CH production: already minimized, loss already maximized
    """
    ...
    # 1. ALL rates with range > 1.5x (EXPANDED from > 2x)
    tunable_range = {
    ...
    # 5. CRITICAL SECONDARY PATHWAYS (from chemistry analysis)
    critical_secondary = [
        # H chemistry (TOP LOSS - must reduce to increase H!)
        'H_CH4_CH3_H2_cm3_7_25',       # H+CH4→CH3+H2 (75.4% of H loss!) ← KEY!
        'stick_H_9_1',                  # H wall sticking (23.4% of H loss)
        'CH3_H_CH2_H2_cm3_7_36',       # CH3+H→CH2+H2 (1.1% of H loss)

        # C2 chemistry (TOP LOSS - must reduce to increase C2!)
        'stick_C2_9_9',                 # C2 wall sticking (63.4% of C2 loss!) ← KEY!
        'C2_H_CH_C_cm3_7_6',           # C2+H→CH+C (18.5% of C2 loss + 17.3% of CH production)
        'loss_C2_11_3',                 # C2 volume loss (18.1% of C2 loss!) ← KEY!

        # C2 production (TOP - must enhance to increase C2!)
        'C2H2_H_C2_H2_H_cm3_7_50',     # C2H2+H→C2 (95.5% of C2 production) ← KEY!
        'CH2_CH2_C2_H2_H2_cm3_7_58',   # CH2+CH2→C2 (2.9% of C2 production)

        # CH chemistry (already at bounds, but include for completeness)
        'CH2_H_CH_H2_cm3_7_1',          # CH2+H→CH (54.2% of CH production)
        'C_H_CH_cm3_7_12',              # C+H→CH (9.2% of CH production)
        'CH_CH4_C2H4_H_cm3_7_20',      # CH+CH4→C2H4 (22.6% of CH loss)
        'CH_CH4_CH2_CH3_cm3_7_39',     # CH+CH4→CH2+CH3 (22.6% of CH loss)
        'loss_CH_11_9',                 # CH volume loss (9.8% of CH loss)
        'stick_CH_9_3',                 # CH wall sticking (7.8% of CH loss)
    ]
    selected |= set(critical_secondary)

    # 6. Target-specific rates (from database helper)
    ...
    # 7. Flagged rates (uncertain in literature)
    ...
    print(f"  V5 COMPREHENSIVE selection: {len(selected_names)} rates")
...
def objective_function(x, param_names, params_base):
    ...
    weights = {
        'H': 3.0,    # Increased from 1.0 (H is 0.40x, need to increase)
        'CH': 30.0,  # Reduced from 40.0 (approaching limits, balance with H & C2)
        'C2': 5.0    # Increased from 3.0 (C2 is 0.32x, need to increase)
    }
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_comprehensive_v5/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
...
def create_warm_start_population(param_names, bounds, db, popsize=6):
    """
    Create initial population biased for CH reduction.

    Strategy for CH optimization (from V3 f=618 best):
    - E ~ 50-100 V/cm (V3 best had E=75 V/cm)
    - Ne ~ 1.3e9 (V3 best had Ne=1.32e9)
    - Minimize CH production: CH2+H→CH, C2+H→CH, C+H→CH
    - Maximize CH loss: CH wall sticking, CH+CH4 reactions
    """
    print("\n  Creating warm-start population biased for CH REDUCTION...")

    population = np.zeros((popsize, len(bounds)))

    # CH production reactions - bias toward MINIMUM
    ch_production = {
        'CH2_H_CH_H2_cm3_7_1',    # CH2+H→CH (48.7% of CH production)
        'C2_H_CH_C_cm3_7_6',      # C2+H→CH (24% of CH production)
        'C_H_CH_cm3_7_12',        # C+H→CH (12.9% of CH production)
    }

    # CH loss reactions - bias toward MAXIMUM
    ch_loss = {
    ...
    for i in range(popsize):
        for j, (name, (low, high)) in enumerate(zip(param_names, bounds)):
            if name == 'E_field':
                # Bias toward HIGHER E (50-100 V/cm) from V3 best
                population[i, j] = np.random.uniform(max(50, low), min(100, high))
            elif name == 'ne':
                # Bias toward V3 best: Ne ~ 1.3e9
                population[i, j] = np.random.uniform(max(1.2e9, low), 1.5e9)
            elif name in ch_production:
                # Minimize CH production - bias toward minimum
                range_width = high - low
                population[i, j] = np.random.uniform(low, low + 0.2 * range_width)
            elif name in ch_loss:
                # Maximize CH loss - bias toward maximum
                range_width = high - low
    ...
    print("  ✓ Population biased: E~50-100 V/cm, Ne~1.3e9, CH production MIN, CH loss MAX")
    return population
...
def main():
    ...
    print("=" * 80)
    print(" V5 OPTIMIZATION - COMPREHENSIVE MULTI-TARGET STRATEGY")
    print("=" * 80)
    print("\nV5 STRATEGY (per user request - OPTIMIZE ALL TARGETS!):")
    print("  ✓ 150-200 tunable rates (ALL rates with range > 1.5x)")
    print("  ✓ Balanced weights: H=3x, CH=30x, C2=5x")
    print("  ✓ Chemistry-guided: Secondary pathways identified")
    print("  ✓ Charge penalty: 75x (moderate balance)")
    print("  ✓ Longer run: 20 iterations (vs 15 in V4)")
    print("  ✓ Ionization rates FIXED (per user guidance)")
    print("\nV4 result: H=0.40x, CH=5.30x, C2=0.32x")
    print("V5 goal: Get ALL THREE closer to 1.0x simultaneously!")
    print("\nKey secondary pathways to optimize:")
    print("  • H loss: H+CH4→CH3+H2 (75%!) and H wall (23%)")
    print("  • C2 loss: C2 wall (63%!), C2+H→CH (18.5%), C2 volume loss (18%)")
    print("  • CH: already at physical limits, need balanced approach")
    ...
    print("=" * 80)
    print("\nDetailed logs will be saved to optimization_results_comprehensive_v5/")
    ...
    result = differential_evolution(
        ...
        strategy='best1bin',
        maxiter=20,   # Longer run for comprehensive optimization
    ...
    with open('optimization_results_comprehensive_v5/FINAL_RESULT.json', 'w') as f:
        json.dump({
    ...
    print(f"\n✓ Final parameters saved to optimization_results_comprehensive_v5/FINAL_RESULT.json")
    print(f"✓ Best result logs saved to optimization_results_comprehensive_v5/best_iteration_*.json")
