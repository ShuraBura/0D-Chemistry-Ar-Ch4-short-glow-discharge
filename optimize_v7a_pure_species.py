#!/usr/bin/env python3
"""
V7a: PURE SPECIES OPTIMIZATION - No charge balance constraint

STRATEGY: Ignore charge balance completely, focus 100% on hitting species targets.
This will reveal the best possible species accuracy without charge constraints.
...
3. NO CHARGE BALANCE PENALTY:
   - CHARGE_BALANCE_WEIGHT = 0 (vs 300 in V6)
   - Pure focus on H, CH, C2 targets

4. MORE ITERATIONS:
   - 25 iterations (vs 20 in V6) for better convergence

Expected result: Best possible species accuracy, likely poor charge balance.
"""
...
CHARGE_BALANCE_WEIGHT = 0.0  # *** NO CHARGE PENALTY ***

os.makedirs('optimization_results_v7a_pure_species', exist_ok=True)
...
def select_tunable_rates():
    """V7a: COMPREHENSIVE + secondary/tertiary pathways"""
...
def objective_function(x, param_names, params_base):
    ...
    # PURE SPECIES ERROR - NO CHARGE PENALTY
    ...
    # Calculate charge balance for reporting only (not in objective!)
    ...
    if error < best_result['objective']:
        ...
        log_file = f'optimization_results_v7a_pure_species/best_iteration_{iteration_counter:04d}_f{error:.1f}.json'
        ...
        print(f"      Ion/e: {ion_e_ratio:.2f}x (not penalized, for info only)")
...
def main():
    print("=" * 80)
    print(" V7a OPTIMIZATION - PURE SPECIES FOCUS (NO CHARGE BALANCE)")
    print("=" * 80)
    print("\nSTRATEGY:")
    print("  ✓ NO charge balance penalty (CHARGE_WEIGHT = 0)")
    print("  ✓ 100% focus on H, CH, C2 targets")
    ...
    print("\nExpected: Best species accuracy, likely poor charge balance")
    ...
    print("\n" + "=" * 80)
    print(" Running V7a Optimization (25 iterations, pop=6)")
    print("=" * 80)
    ...
    print(f"\nV7a COMPLETE ({elapsed/60:.1f} minutes)")
    print(f"Final objective: {result.fun:.6f}")

    with open('optimization_results_v7a_pure_species/FINAL_RESULT.json', 'w') as f:
        json.dump({
