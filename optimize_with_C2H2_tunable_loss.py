#!/usr/bin/env python3
"""
OPTIMIZATION WITH C2H2 CONSTRAINT + TUNABLE C2H2 LOSS RATES

CRITICAL INSIGHT from user:
C2 CAN outnumber CH by 2 orders of magnitude IF C2H2 ≥ 5e+12 to 1e+13 cm⁻³

Key reactions that drive C2 production with high C2H2:
1. CH + CH → C2 + H2
2. e + C2H2 → C2H + H + e⁻ (dissociative ionization)
3. e + C2H → C2 (subsequent step)

CONSTRAINTS:
1. C2H2 ≥ 5e+12 cm⁻³ (REQUIRED for C2 pathway)
2. CH < 2e+09 cm⁻³ (prevent CH overshoot)
3. Equal weights H:CH:C2 = 20:20:20
4. Charge balance Ni/Ne in [2, 7]
5. Integration to t=500s

NEW IN THIS VERSION:
- Force-includes C2H2 loss rates in tunable set:
  * stick_C2H2_9_11 (wall sticking)
  * loss_C2H2_11_19 (volumetric loss)
- Allows optimizer to reduce C2H2 losses to reach 5e+12 target
"""
...
# Create results directory for C2H2-tunable-loss version
os.makedirs('optimization_results_C2H2_tunable_loss', exist_ok=True)
...
def select_tunable_rates():
    """Select ~50 most important tunable rates (expanded for C2 production + C2H2 control)."""
    ...
    critical_reactions = [
        ...
        # FORCE-INCLUDE: Critical C2-producing reactions
        ...
        # NEW: FORCE-INCLUDE C2H2 loss rates (CRITICAL for reaching C2H2 ≥ 5e+12!)
        'stick_C2H2_9_11',             # C2H2 wall sticking (allowed: 500-2000 s⁻¹)
        'loss_C2H2_11_19',             # C2H2 volumetric loss (allowed: 1000-2000 s⁻¹)
    ]
...
def run_simulation(rate_values, Te, ne, E_field, params_base, log_file=None):
    ...
    try:
        ...
        # Integration to TRUE STEADY STATE
        # CRITICAL FIX: t=100s was too short - stopped at transient state!
        # Testing showed at t=100s: H=1e+13, but at t=1000s: H=3.66e+14 (34× higher!)
        # Now integrate to t=500s to ensure we reach true steady state
        sol = solve_ivp(
            ode_func,
            (0, 500),       # INCREASED from 100s - reaches true steady state
            y0,
            method='BDF',
            rtol=1e-7,
            atol=1e-9,
            max_step=1.0    # Can use larger step for longer integration
        )
        ...
        # DEBUG: Print integration endpoint and H evolution
        H_idx_debug = species.index('H')
        H_initial = sol.y[H_idx_debug, 0]
        H_final_integration = sol.y[H_idx_debug, -1]
        print(f"  [DEBUG] t_final={sol.t[-1]:.2f}s, nsteps={len(sol.t)}")
        print(f"  [DEBUG] H: {H_initial:.2e} → {H_final_integration:.2e} (factor: {H_final_integration/H_initial:.1f}×)")

        y_final = sol.y[:, -1]

        # DEBUG: Verify y_final is actually the final timestep
        H_from_y_final = y_final[H_idx_debug]
        print(f"  [DEBUG] H from y_final: {H_from_y_final:.2e} (match: {abs(H_from_y_final - H_final_integration) < 1e6})")
        ...
        results = {
            ...
            'CH': get_density('CH'),
            'C2': get_density('C2'),
            'C2H2': get_density('C2H2'),  # CRITICAL for C2 production pathway
            'n_i_total': n_i_total,
...
def objective_function(x, param_names, params_base):
    ...
    # Species target errors - Equal weights for all species
    weights = {
        'H': 20.0,
        'CH': 20.0,
        'C2': 20.0
    ...
    # C2H2 CONSTRAINT - CRITICAL for C2 production pathway!
    # User insight: Need C2H2 ≥ 5e+12 cm⁻³ for reactions like:
    #   e + C2H2 → C2H + H, then e + C2H → C2
    #   CH + CH → C2 + H2 (requires high CH from C2H2 chemistry)
    C2H2_min_required = 5e12
    C2H2_actual = results['C2H2']
    if C2H2_actual < C2H2_min_required:
        # Large penalty if C2H2 too low
        C2H2_penalty = 100.0 * ((C2H2_min_required - C2H2_actual) / C2H2_min_required)**2
    else:
        C2H2_penalty = 0.0  # No penalty if C2H2 ≥ 5e+12

    # CH CONSTRAINT - Prevent CH overshoot that consumes all carbon
    CH_max_allowed = 2e9  # 2× target maximum
    CH_actual = results['CH']
    if CH_actual > CH_max_allowed:
        # Penalty for CH overshoot
        CH_penalty = 50.0 * ((CH_actual - CH_max_allowed) / CH_max_allowed)**2
    else:
        CH_penalty = 0.0

    # Total error
    total_error = species_error + charge_penalty + C2H2_penalty + CH_penalty
    ...
    if total_error < best_result['objective']:
        ...
        log_file = f'optimization_results_C2H2_tunable_loss/best_f{total_error:.1f}.json'
        ...
        print(f"      C2H2: {results['C2H2']:.2e} (required: ≥5e+12 for C2 pathway)")
        print(f"      Penalties: species={species_error:.2f}, charge={charge_penalty:.2f}, C2H2={C2H2_penalty:.2f}, CH={CH_penalty:.2f}")
