#!/usr/bin/env python3
"""
Ne sweep exploring higher values (1e8 is too low)
Current best: Te=1.20 eV, E=75 V/cm, Ne=1.0e8
Goal: Explore higher Ne to see if CH control improves
"""
...
# Ne sweep - exploring higher values
ne_values = [
    # Low baseline for comparison
    1.0e8, 1.5e8, 2.0e8, 2.5e8,
    # Medium range
    3e8, 4e8, 5e8, 6e8, 7e8, 8e8, 9e8,
    # Higher range (exploring if better)
    1e9, 1.5e9, 2e9, 2.5e9, 3e9,
    4e9, 5e9, 6e9, 7e9, 8e9, 9e9,
    # Very high
    1e10, 1.5e10, 2e10
]

print("="*80)
print("NE SWEEP: EXPLORING HIGHER VALUES")
print("="*80)
print()
...
print()
print("Rationale: 1e8 might be too low, exploring higher Ne")
...
if results:
    ...
    # CH vs Ne trend
    print("CH TREND WITH NE:")
    print(f"{'Ne (cm⁻³)':>12} | {'CH ratio':>9} | {'Ni/Ne':>6} | {'f(x)':>7}")
    print("-"*50)
    for r in results:
        ni_ne_ok = "✓" if 2.0 <= r['ni_ne_ratio'] <= 6.0 else " "
        print(f"{r['ne']:12.1e} | {r['CH_ratio']:9.2f} | {r['ni_ne_ratio']:5.2f}{ni_ne_ok} | {r['objective']:7.1f}")
...
# Save results
with open('ne_higher_sweep_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Results saved to: ne_higher_sweep_results.json")
