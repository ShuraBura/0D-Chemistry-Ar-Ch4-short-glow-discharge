#!/usr/bin/env python3
"""
Ne sweep with ALL current optimizations at best Te/E conditions
Goal: Find Ne that minimizes CH while maintaining H, C2, and Ni/Ne
"""
...
# BEST Te/E from previous sweep
TE_BEST = 1.20  # Found to give lowest CH
E_BEST = 75     # Middle of best range

# Ne values to test - wide range
ne_values = [
    3e8, 4e8, 5e8, 6e8, 7e8, 8e8,
    1e9, 1.5e9, 2e9, 2.5e9, 3e9,
    4e9, 5e9, 6e9, 8e9, 1e10
]

print("="*80)
print("Ne SWEEP WITH FULL OPTIMIZATIONS")
print("="*80)
print()
print(f"Fixed: Te={TE_BEST} eV, E={E_BEST} V/cm (best from Te/E sweep)")
...
if results:
    ...
    # Results with good Ni/Ne
    good_ni_ne = [r for r in results if 2.0 <= r['ni_ne_ratio'] <= 6.0]
    if good_ni_ne:
        print(f"Results with Ni/Ne in target range (2-6×): {len(good_ni_ne)}/{len(results)}")
        print()
        print("ALL results with good Ni/Ne:")
        print(f"{'Ne':>10} | {'Ni/Ne':>7} | {'f(x)':>7} | {'H':>6} | {'CH':>7} | {'C2':>6}")
        print("-"*70)
        for r in sorted(good_ni_ne, key=lambda x: x['objective']):
            print(f"{r['ne']:>10.2e} | {r['ni_ne_ratio']:>7.2f} | {r['objective']:>7.1f} | "
...
with open('ne_sweep_full_opt_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Detailed results saved to: ne_sweep_full_opt_results.json")
