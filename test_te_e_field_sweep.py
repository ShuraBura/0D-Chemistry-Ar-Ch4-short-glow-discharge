#!/usr/bin/env python3
"""
Comprehensive test of Te and E_field effects on:
1. Target species (H, CH, C2)
2. Ion density (Ni) and Ni/Ne ratio
3. Strategy: Higher E increases ion drift losses → reduces Ni
4. Lower Te reduces electron-impact CH production (e + CH4 → CH + ...)
"""
...
# Create test matrix: Te and E_field combinations
te_values = [0.8, 1.0, 1.2, 1.47, 1.8, 2.0]  # eV (1.47 is checkpoint)
e_field_values = [200, 300, 400, 500, 600]  # V/cm (higher E = more ion drift)

print("="*80)
print("Te & E_field SWEEP - Impact on densities and charge balance")
print("="*80)
print()
print("Strategy:")
print("  1. Higher E_field → more ion drift losses → lower Ni")
print("  2. Lower Te → less e + CH4 → CH production")
print("  3. Find optimal Te, E for target densities AND proper Ni/Ne ratio")
print()
print(f"Testing Te values: {te_values} eV")
print(f"Testing E_field values: {e_field_values} V/cm")
print(f"Fixed Ne: 3.0e9 cm⁻³ (user's educated guess)")
print(f"Target Ni/Ne: 2-6× (sheath edge)")
print("="*80)
print()
...
for te in te_values:
    for e_field in e_field_values:
        ...
        try:
            ...
            print(f" {in_range} Ni/Ne={ni_ne_ratio:.2f} | f(x)={objective:.0f} | H={H/TARGETS['H']:.2f}× | C2={C2/TARGETS['C2']:.2f}× | CH={CH/TARGETS['CH']:.1f}×")
...
if results:
    ...
    print("="*80)
    print("SUMMARY TABLE (top 10 by objective)")
    print("="*80)
    print(f"{'Te':>6} | {'E':>5} | {'Ni/Ne':>7} | {'f(x)':>8} | {'H':>6} | {'C2':>6} | {'CH':>7}")
    print("-"*80)

    for r in sorted(results, key=lambda x: x['objective'])[:10]:
        in_range = "✓" if 2.0 <= r['ni_ne_ratio'] <= 6.0 else "✗"
        print(f"{r['te']:>6.2f} | {r['e_field']:>5} | {r['ni_ne_ratio']:>6.2f}{in_range} | {r['objective']:>8.0f} | "
...
with open('te_e_field_sweep_results.json', 'w') as f:
    json.dump(results, f, indent=2)
print("Detailed results saved to: te_e_field_sweep_results.json")
