#!/usr/bin/env python3
"""
Check if best steady-state result has reached true steady state
"""
...
# Load best steady-state result
with open('optimization_results_charge_balanced_steady_state/best_f117.8.json', 'r') as f:
    best = json.load(f)

print("=" * 80)
print("STEADY STATE VERIFICATION - BEST RESULT")
print("=" * 80)
print()
...
if abs(dydt_final[idx_H]/H_final) < 1e-10:
    ...
    print(f"⚠ H NOT at steady state (relative rate: {abs(dydt_final[idx_H]/H_final):.2e})")

if abs(dydt_final[idx_CH]/CH_final) < 1e-10:
    print("✓ CH at true steady state")
else:
    print(f"⚠ CH NOT at steady state (relative rate: {abs(dydt_final[idx_CH]/CH_final):.2e})")

if abs(dydt_final[idx_C2]/C2_final) < 1e-10:
    print("✓ C2 at true steady state")
else:
    print(f"⚠ C2 NOT at steady state (relative rate: {abs(dydt_final[idx_C2]/C2_final):.2e})")
